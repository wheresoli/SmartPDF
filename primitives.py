...
def detect_all_text_regions(img: np.ndarray, color: Color | None = None, ocr_enabled: bool = True,
                            h_kernel: int = 6, merge_gap: int = 20, min_width: int = 10,
                            min_area: int = 50) -> List[Primitive]:
    """Detect ALL text regions using multiple methods.

    Args:
        img: Input image
        color: Optional color filter
        ocr_enabled: Whether to use OCR-based detection (morphological)
        h_kernel: Horizontal kernel size for morphological dilation
        merge_gap: Maximum gap in pixels to merge nearby segments
        min_width: Minimum width for a valid text region
        min_area: Minimum area for a valid text region
    """
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if img.ndim == 3 else img.copy()
    ...
    if not ocr_enabled:
        # Return empty list if OCR is disabled
        return text_regions
    ...
    try:
        ...
        # Kernel size is now parameterized
        h_kernel_struct = cv2.getStructuringElement(cv2.MORPH_RECT, (h_kernel, 1))
        h_dilate = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, h_kernel_struct, iterations=1)
        ...
        for c in cnts:
            ...
            if h > 80 or w > 1000:
                ...
                for line_start, line_end in zip(starts, ends):
                    ...
                    # Merge nearby segments (parameterized gap) to avoid over-fragmentation
                    merged_segs = []
                    if len(seg_starts) > 0:
                        ...
                        for i in range(1, len(seg_starts)):
                            ...
                            if gap < merge_gap:  # Merge if gap is small (parameterized)
                    ...
                    for seg_start, seg_end in merged_segs:
                        ...
                        if seg_w < min_width:  # Too narrow (parameterized)
                            continue
                        if seg_w * line_h < min_area:  # Too small (parameterized)
                            continue
            ...
            # Normal-sized regions: apply standard filters (parameterized)
            if w < min_width or h < 5:
                continue
            if w * h < min_area:
                continue
